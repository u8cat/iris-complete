...
Lemma resolve_reducible e σ (p : proph_id) v :
  Atomic StronglyAtomic e → reducible e σ → p ∈ σ.(used_proph_id) →
...
Proof.
  ...
  simpl. constructor; first assumption. by apply prim_step_to_val_is_base_step.
Qed.
...
(* TODO: move to proph_map.v *)
Lemma proph_map_agree pvs ps p vs :
  proph_map_interp pvs ps -∗ proph p vs -∗
  ⌜ p ∈ ps ∧ vs = proph_list_resolves pvs p⌝.
Proof.
  iIntros "Hpm Hp◯".
  rewrite proph_map.proph_unseal /proph_map.proph_def.
  iDestruct "Hpm" as (R) "[[%Hresolve %Hdom] Hp●]".
  iCombine "Hp● Hp◯" gives %Hlookup.
  iPureIntro. split; last auto.
  eapply elem_of_weaken; last eassumption.
...
Proof.
  ...
  iDestruct (proph_map_agree with "Hκ Hp") as %[Hin _].
